...
interface CancelAppeal {
    id: number;
}

export const Cancel: React.FC<CancelAppeal> = ({ id }) => {
    ...
    const handleCancelSubmit = async (id: number) => {
        try {
            const data = await fetch(`http://localhost:3001/appeals/${id}/cancel`,
                {
                    method: 'PUT',
    ...
    return (
        <div key={id}>
            <Popup trigger={<button style={{ cursor: 'pointer', backgroundColor: '#FF6F6F', border: 'none', borderRadius: '0 0 8px 8px', width: '15em', height: '2.2em', fontSize: '15px' }}>Отменить обращение</button>} modal nested>
                {!isModalOpen && (
                    <div className="modal" style={{ display: 'grid', justifyContent: 'center', alignItems: 'center', minWidth: '300px', minHeight: '150px', backgroundColor: "#A8A8A8", marginBottom: "12px", borderRadius: "12px", padding: "8px" }}>
                        ...
                        />
                        <div>
                            <button type="submit" style={{ cursor: 'pointer', border: 'none', borderRadius: '8px', width: '15em', height: '2em', fontSize: '14px'}} onClick={() => { handleCancelSubmit(id); setIsModalOpen(true) }}>Отправить</button>
                        </div>
                    </div>
                )}
            </Popup>
